...
for api in args.api or ["openai-completions", "openai-responses", "anthropic-messages"]:
    # Pi preserves Responses call_id/item_id pairs for its OpenAI provider.
    provider_name = "openai" if api == "openai-responses" else "q36"
    project = out / api
    ...
    provider = {"baseUrl": base + ("" if api == "anthropic-messages" else "/v1"),
                ...
                "models": [{"id": "qwen3.6-35b-a3b-nothink", "name": "Vision QA", "reasoning": True,
                            "input": ["text", "image"], "contextWindow": 16384, "maxTokens": 1536,
                            "cost": {"input": 0, "output": 0, "cacheRead": 0, "cacheWrite": 0}}]}
    if api == "openai-completions":
        provider["compat"] = {"supportsDeveloperRole": False}
    (settings / "models.json").write_text(json.dumps({"providers": {provider_name: provider}}))
    for name in ["text.png", "spatial.png"]:
        shutil.copyfile(root / "tests/vision-fixtures" / name, project / name)
    ...
    cmd = [str(args.pi.resolve()),
           "--print", "--mode", "json", "--provider", provider_name, "--model", "qwen3.6-35b-a3b-nothink",
