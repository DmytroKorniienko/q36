 ...
 * input/output, while the worker thread owns the live Q36 session and KV state.
 * These types define the shared state and the small streaming state machines
 * used to render sampled assistant text and Qwen tool calls as they arrive.
 */
...
typedef struct {
    ...
    size_t part_count;
    size_t part_cap;
    q36_vision_embedding *images;
    size_t image_count;
    size_t image_cap;
} agent_tool_observation;

typedef enum {
    AGENT_QWEN_TOOL_SEARCH,
    AGENT_QWEN_TOOL_STRUCTURAL,
    AGENT_QWEN_TOOL_PARAM_VALUE,
    AGENT_QWEN_TOOL_DONE,
    AGENT_QWEN_TOOL_ERROR,
} agent_qwen_tool_state;

typedef struct {
    agent_qwen_tool_state state;
    char search_tail[64];
    size_t search_len;
    char *raw;
    size_t raw_len;
...
typedef struct {
    char tail[32];
    size_t len;
} agent_qwen_tool_marker_detector;

typedef struct {
    agent_token_renderer *renderer;
    agent_qwen_tool_parser *parser;
    agent_tool_visualizer viz;
    bool in_think;
    bool qwen_tool_active;
    bool qwen_tool_ignored;
    bool replay;
    char pending[16];
    size_t pending_len;
    char qwen_tool_start_tail[64];
    size_t qwen_tool_start_len;
    agent_qwen_tool_marker_detector plain_qwen_tool;
    agent_qwen_tool_marker_detector think_qwen_tool;
    bool qwen_tool_in_think;
    bool qwen_tool_in_think_reported;
    bool post_think_gap;
...
static void agent_worker_images_clear(agent_worker *w) {
    ...
    for (size_t i = 0; i < w->image_count; i++)
        q36_vision_embedding_free(&w->images[i].embedding);
    free(w->images);
    w->images = NULL;
    w->image_count = 0;
    w->image_cap = 0;
...
static int parse_int(const char *s, const char *opt) {
    ...
    if (s[0] == '\0' || *end != '\0' || v <= 0 || v > INT32_MAX) {
        fprintf(stderr, "q36-agent: invalid value for %s: %s\n", opt, s);
        exit(2);
    }
    return (int)v;
}
...
static q36_backend parse_backend(const char *s) {
    ...
    fprintf(stderr, "q36-agent: invalid backend: %s\n", s);
    fprintf(stderr, "q36-agent: valid backends are: metal, vulkan, cpu\n");
    exit(2);
}
...
static void usage(FILE *fp, const char *topic) {
    q36_help_print(fp, Q36_HELP_AGENT, topic);
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-p") || !strcmp(arg, "--prompt")) {
            ...
            if (!q36_parse_kv_cache_type(need_arg(&i, argc, argv, arg), &c.engine.cache_type_k)) {
                fprintf(stderr, "q36-agent: invalid KV cache K type\n");
                exit(2);
            }
            ...
            if (!q36_parse_kv_cache_type(need_arg(&i, argc, argv, arg), &c.engine.cache_type_v)) {
                fprintf(stderr, "q36-agent: invalid KV cache V type\n");
                exit(2);
            }
            ...
            c.engine.backend = Q36_BACKEND_CPU;
                   ...
                   !strcmp(arg, "--role") || !strcmp(arg, "--layers") ||
                   !strcmp(arg, "--listen") || !strcmp(arg, "--coordinator")) {
            fprintf(stderr, "q36-agent: %s is not supported in this q36 port\n", arg);
            exit(2);
            ...
            if (!q36_parse_streaming_cache_experts_arg(
                ...
                fprintf(stderr,
                        "q36-agent: --ssd-streaming-cache-experts must be a positive count or <number>GB\n");
                exit(2);
            }
            ...
            if (v <= 0) {
                fprintf(stderr, "q36-agent: --ssd-streaming-preload-experts must be positive\n");
                exit(2);
            }
            ...
            if (!q36_parse_gib_arg(need_arg(&i, argc, argv, arg),
                ...
                fprintf(stderr,
                        "q36-agent: --simulate-used-memory must be a positive GiB value, e.g. 64GB\n");
                exit(2);
            }
        } else if (!strcmp(arg, "--prefill-chunk")) {
            int v = parse_int(need_arg(&i, argc, argv, arg), arg);
            if (v <= 0) {
                fprintf(stderr, "q36-agent: --prefill-chunk must be positive\n");
                exit(2);
            }
            ...
            if (c.engine.power_percent < 1 || c.engine.power_percent > 100) {
                fprintf(stderr, "q36-agent: --power must be between 1 and 100\n");
                exit(2);
            }
            ...
            fprintf(stderr, "q36-agent: unknown option: %s\n", arg);
            usage(stderr, NULL);
            exit(2);
        }
    }
...
static const char agent_tools_prompt_vision[] =
    "{\n"
    "  \"type\": \"function\",\n"
    "  \"function\": {\n"
    "    \"name\": \"view_image\",\n"
...
static char *agent_build_tools_prompt(bool edit_upto, bool vision) {
    const char *edit = edit_upto ? agent_tools_prompt_edit_upto : agent_tools_prompt_edit_exact;
    const char *tail = strstr(agent_tools_prompt_after_edit, "</tools>");
    size_t a = strlen(agent_tools_prompt_intro), b = strlen(edit);
    size_t c = (size_t)(tail - agent_tools_prompt_after_edit);
    size_t d = vision ? strlen(agent_tools_prompt_vision) : 0;
    char *out = xmalloc(a + b + c + d + strlen(tail) + 1);
    memcpy(out, agent_tools_prompt_intro, a);
    memcpy(out + a, edit, b);
    memcpy(out + a + b, agent_tools_prompt_after_edit, c);
    if (vision) memcpy(out + a + b + c, agent_tools_prompt_vision, d);
    strcpy(out + a + b + c + d, tail);
    return out;
}

static char *agent_build_system_prompt(const char *extra, bool *thinking,
                                       bool edit_upto, bool vision) {
    char *tools = agent_build_tools_prompt(edit_upto, vision);
...
static void agent_worker_append_hints_context(agent_worker *w) {
    ...
    if (!note) return;
    q36_chat_append_message(w->engine, &w->transcript, "system", note);
...
static char *agent_build_system_prompt_reminder(bool edit_upto, bool vision) {
    char *tools = agent_build_tools_prompt(edit_upto, vision);
...
static void agent_worker_maybe_append_datetime_context(agent_worker *w) {
    ...
    q36_chat_append_message(w->engine, &w->transcript, "system", msg);
    agent_trace_text(w, "datetime-context", msg, strlen(msg));
    w->datetime_context_injected = true;
}

/* Refresh the trusted tool prompt in very long sessions. */
static void agent_worker_maybe_append_system_prompt_reminder(agent_worker *w) {
    ...
    char *reminder = agent_build_system_prompt_reminder(w->cfg->edit_upto,
                                                        q36_engine_has_vision(w->engine));
...
static void agent_trace_tokens(agent_worker *w, const char *label,
                               const q36_tokens *tokens, int start) {
    ...
    for (int i = start; i < tokens->len; i++) {
        size_t text_len = 0;
        char *text = q36_token_text(w->engine, tokens->v[i], &text_len);
...
/* ============================================================================
 * Qwen3.6 Tool-Call Parser
 * ============================================================================
 *
 * The model streams the format embedded in its chat template:
 * <tool_call><function=name><parameter=key>value...</parameter>...</function>
 * </tool_call>.  The parser stays strict once the opening marker is complete.
 */
...
static void agent_qwen_tool_parser_free(agent_qwen_tool_parser *p) {
    if (!p) return;
    free(p->raw);
...
static void agent_qwen_tool_parser_reset(agent_qwen_tool_parser *p) {
    agent_qwen_tool_parser_free(p);
    p->state = AGENT_QWEN_TOOL_SEARCH;
}

static void agent_qwen_tool_raw_append(agent_qwen_tool_parser *p, const char *s, size_t n) {
    if (!n) return;
...
static void agent_qwen_tool_set_error(agent_qwen_tool_parser *p, const char *msg) {
    p->state = AGENT_QWEN_TOOL_ERROR;
    snprintf(p->error, sizeof(p->error), "%s", msg);
}

static bool agent_qwen_tool_close_tag_at(const char *s, const char *name, size_t *tag_len) {
    char prefix[64];
 ...
 * handled by agent_qwen_tool_close_tag_at(); this helper exists for online behavior:
 * terminal rendering must hide partial close tags without waiting for the whole
 * parameter to finish. */
static bool agent_qwen_tool_parameter_close_tail(const char *tail, size_t len,
                                            bool *complete) {
...
static void agent_qwen_tool_update_param_close_prefix(agent_qwen_tool_parser *p) {
    p->param_close_prefix = false;
    if (p->state != AGENT_QWEN_TOOL_PARAM_VALUE || p->raw_len <= p->param_value_start)
        return;
    ...
    bool complete = false;
    static const char qwen_tool_marker[] = "</parameter";
    p->param_close_prefix =
        tail_len >= sizeof(qwen_tool_marker) - 1 &&
        memcmp(lt, qwen_tool_marker, sizeof(qwen_tool_marker) - 1) == 0 &&
        agent_qwen_tool_parameter_close_tail(lt, tail_len, &complete) &&
        !complete;
}

/* Find a Qwen tool closing tag while accepting the few harmless closing-tag variants
 * the model has been observed to emit.  Opening tags stay strict so accidental
 * prose does not become a tool call. */
static char *agent_qwen_tool_find_close_tag(const char *s, const char *name, size_t *tag_len) {
    const char *p = s;
    while ((p = strstr(p, "</")) != NULL) {
        if (agent_qwen_tool_close_tag_at(p, name, tag_len)) return (char *)p;
        p++;
    }
    return NULL;
}

/* Parse as much of the accumulated Qwen tool buffer as possible.  The parser can be
 ...
 * AGENT_QWEN_TOOL_ERROR so the model gets a retryable tool error. */
static void agent_qwen_tool_parse(agent_qwen_tool_parser *p) {
    while (p->state == AGENT_QWEN_TOOL_STRUCTURAL || p->state == AGENT_QWEN_TOOL_PARAM_VALUE) {
        if (p->state == AGENT_QWEN_TOOL_PARAM_VALUE) {
            size_t end_tag_len = 0;
            char *end = agent_qwen_tool_find_close_tag(p->raw + p->param_value_start,
            ...
            agent_tool_call_add_arg(&p->current, p->param_name ? p->param_name : "",
                                    value, value_len);
            q36_tool_text_unescape(p->current.args[p->current.argc - 1].value, "</parameter>");
            p->param_close_prefix = false;
            free(p->param_name);
            p->param_name = NULL;
            p->parse_pos = (size_t)(end - p->raw) + end_tag_len;
            p->state = AGENT_QWEN_TOOL_STRUCTURAL;
            continue;
        }
        ...
        size_t close_len = 0;
        if (agent_qwen_tool_close_tag_at(p->raw + p->parse_pos, "tool_call", &close_len)) {
            if (p->current.name) {
                agent_qwen_tool_set_error(p, "tool call closed before function");
                return;
            }
            if (p->calls.len == 0) {
                agent_qwen_tool_set_error(p, "tool call has no function");
                return;
            }
            p->parse_pos += close_len;
            p->state = AGENT_QWEN_TOOL_DONE;
            return;
        }
        if (agent_qwen_tool_close_tag_at(p->raw + p->parse_pos, "function", &close_len)) {
            if (!p->current.name) {
                agent_qwen_tool_set_error(p, "function close without function");
                return;
            }
        ...
        if (!strncmp(tag, "<function=", strlen("<function="))) {
            agent_tool_call_free(&p->current);
            const char *name = tag + strlen("<function=");
            size_t name_len = tag_len - strlen("<function=") - 1;
            while (name_len && isspace((unsigned char)name[name_len - 1])) name_len--;
            p->current.name = xstrndup(name, name_len);
            if (!p->current.name[0]) {
                free(tag);
                agent_qwen_tool_set_error(p, "function without name");
                return;
            }
            p->parse_pos += tag_len;
        } else if (!strncmp(tag, "<parameter=", strlen("<parameter="))) {
            if (!p->current.name) {
                free(tag);
                agent_qwen_tool_set_error(p, "parameter outside function");
                return;
            }
            free(p->param_name);
            const char *name = tag + strlen("<parameter=");
            size_t name_len = tag_len - strlen("<parameter=") - 1;
            while (name_len && isspace((unsigned char)name[name_len - 1])) name_len--;
            p->param_name = xstrndup(name, name_len);
            if (!p->param_name[0]) {
                free(tag);
                agent_qwen_tool_set_error(p, "parameter without name");
                return;
            }
            p->parse_pos += tag_len;
            p->param_value_start = p->parse_pos;
            p->param_close_prefix = false;
            p->state = AGENT_QWEN_TOOL_PARAM_VALUE;
        } else {
            snprintf(p->error, sizeof(p->error), "unexpected Qwen tool tag: %.*s",
...
static void agent_qwen_tool_start(agent_qwen_tool_parser *p) {
    static const char start[] = "<tool_call>";
    p->state = AGENT_QWEN_TOOL_STRUCTURAL;
    p->search_len = 0;
    agent_qwen_tool_raw_append(p, start, strlen(start));
    p->parse_pos = strlen(start);
}

static void agent_qwen_tool_feed(agent_qwen_tool_parser *p, const char *s, size_t n) {
    static const char start[] = "<tool_call>";
    const size_t start_len = sizeof(start) - 1;
    if (p->state == AGENT_QWEN_TOOL_DONE || p->state == AGENT_QWEN_TOOL_ERROR) return;

    for (size_t i = 0; i < n; i++) {
        char c = s[i];
        if (p->state == AGENT_QWEN_TOOL_SEARCH) {
            ...
            if (p->search_len >= start_len &&
                ...
                agent_qwen_tool_start(p);
            continue;
        }

        agent_qwen_tool_raw_append(p, &c, 1);
        agent_qwen_tool_parse(p);
        if (p->state == AGENT_QWEN_TOOL_PARAM_VALUE)
            agent_qwen_tool_update_param_close_prefix(p);
        else
            p->param_close_prefix = false;
    }
}
 ...
 * the model is still sampling.  This state machine suppresses raw Qwen tool and
...
static void agent_tool_viz_start(agent_stream_renderer *sr) {
    ...
    if (sr->replay) {
        ...
        /* The raw Qwen tool start marker may arrive after ordinary text on the
...
/* Stream one Qwen tool parameter byte into the visualizer.  The visualizer must not
...
static void agent_tool_viz_param_value_byte(agent_stream_renderer *sr, char c) {
    ...
    if (v->param_end_len || c == '<') {
        ...
        bool complete = false;
        if (agent_qwen_tool_parameter_close_tail(v->param_end_tail, v->param_end_len, &complete)) {
...
static void agent_tool_viz_dump_invalid_qwen_tool(agent_stream_renderer *sr) {
    agent_tool_visualizer *v = &sr->viz;
    if (!v->active) return;

    /* The normal path hides Qwen tool and paints a friendly semantic projection.  If
    ...
    if (sr->parser->raw && sr->parser->raw_len) {
        ...
        agent_tool_viz_puts(sr, "<empty Qwen tool>");
    }
    renderer_color(sr->renderer, "\x1b[0m");
    if (!v->last_output_newline) agent_tool_viz_puts(sr, "\n");
}

static void agent_stream_finish_ignored_qwen_tool(agent_stream_renderer *sr, const char *detail) {
    const char *msg =
        detail && detail[0] ? detail :
        "tool calling is not allowed inside <think></think>";
    sr->qwen_tool_in_think = true;
    sr->qwen_tool_in_think_reported = true;
    agent_trace(sr->renderer->worker, "qwen_tool ignored inside thinking: %s", msg);
    ...
    renderer_plain(sr->renderer, "]\n", 2);
    renderer_color(sr->renderer, "\x1b[0m");
    agent_qwen_tool_parser_reset(sr->parser);
    sr->qwen_tool_active = false;
    sr->qwen_tool_ignored = false;
}

static void agent_stream_malformed_qwen_tool(agent_stream_renderer *sr,
                                        const char *detail) {
    const char *msg = detail && detail[0] ? detail :
        "Qwen tool markup outside a valid tool_call block";
    if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) return;
    agent_qwen_tool_set_error(sr->parser, msg);
    agent_trace(sr->renderer->worker, "malformed qwen_tool in assistant output: %s", msg);
    if (!sr->renderer->last_output_newline)
        renderer_plain(sr->renderer, "\n", 1);
    renderer_color(sr->renderer, "\x1b[1;31m");
...
static void agent_stream_tool_events(agent_stream_renderer *sr) {
    agent_qwen_tool_parser *p = sr->parser;
    agent_tool_visualizer *v = &sr->viz;
    ...
    if (!v->param_active && p->state == AGENT_QWEN_TOOL_PARAM_VALUE && p->param_name)
        agent_tool_viz_param_begin(sr, p->param_name);
}

static void agent_stream_preflight_closed_param(agent_stream_renderer *sr) {
    if (!sr || sr->replay || sr->qwen_tool_ignored || sr->tool_preflight_error)
        return;
    agent_qwen_tool_parser *p = sr->parser;
    agent_tool_visualizer *v = &sr->viz;
    ...
    snprintf(sr->tool_preflight_error_msg, sizeof(sr->tool_preflight_error_msg),
             "edit old selector failed before new was generated: %.200s",
...
static void agent_stream_feed_qwen_tool_byte(agent_stream_renderer *sr, char c) {
    bool was_param = !sr->qwen_tool_ignored && sr->viz.param_active;
    agent_qwen_tool_feed(sr->parser, &c, 1);
    if (!sr->qwen_tool_ignored) {
        agent_stream_tool_events(sr);
        if (was_param) agent_tool_viz_param_value_byte(sr, c);
        if (was_param && sr->parser->state != AGENT_QWEN_TOOL_PARAM_VALUE &&
            sr->viz.param_active)
        {
    ...
    if (sr->parser->state == AGENT_QWEN_TOOL_DONE) {
        if (sr->qwen_tool_ignored) {
            agent_stream_finish_ignored_qwen_tool(
                sr, "tool calling is not allowed inside <think></think>");
        } else {
            agent_trace(sr->renderer->worker, "qwen_tool done calls=%d",
                        sr->parser->calls.len);
            agent_tool_viz_finish(sr, NULL);
            sr->qwen_tool_active = false;
        }
    } else if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) {
        if (sr->qwen_tool_ignored) {
            agent_stream_finish_ignored_qwen_tool(
            ...
            agent_trace(sr->renderer->worker, "qwen_tool error %s",
                        sr->parser->error[0] ? sr->parser->error : "parse error");
            agent_tool_viz_dump_invalid_qwen_tool(sr);
...
/* Start a Qwen3.6 tool block once its complete opening marker is buffered. */
static void agent_stream_start_qwen_tool(agent_stream_renderer *sr, bool ignored) {
    sr->qwen_tool_active = true;
    sr->qwen_tool_ignored = ignored;
    if (ignored) sr->qwen_tool_in_think = true;
    sr->qwen_tool_start_len = 0;
    sr->post_think_gap = false;
    agent_trace(sr->renderer->worker, "qwen_tool start detected%s",
                ignored ? " inside thinking" : "");
    agent_qwen_tool_start(sr->parser);
    if (!ignored) {
...
static void agent_stream_note_plain_qwen_tool_byte(agent_stream_renderer *sr, char c);

static void agent_stream_flush_start_tail(agent_stream_renderer *sr) {
    if (!sr->qwen_tool_start_len) return;
    sr->post_think_gap = false;
    for (size_t i = 0; i < sr->qwen_tool_start_len; i++) {
        renderer_write_char(sr->renderer, sr->qwen_tool_start_tail[i]);
        agent_stream_note_plain_qwen_tool_byte(sr, sr->qwen_tool_start_tail[i]);
        if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) break;
    }
    sr->qwen_tool_start_len = 0;
}

static bool agent_stream_qwen_tool_start_match(const char *tail, size_t len,
                                          bool *complete) {
    static const char canonical[] = "<tool_call>";
    *complete = false;
    size_t canonical_len = sizeof(canonical) - 1;
    if (len <= canonical_len && memcmp(canonical, tail, len) == 0) {
        *complete = len == canonical_len;
        return true;
    }
    return false;
}
...
/* Detect tool-call markup not currently owned by the executable parser. Inside
 * <think> the marker means "tool call attempted too early", while in normal
 * assistant output it becomes a retryable tool error. */
static bool agent_qwen_tool_marker_detector_feed(agent_qwen_tool_marker_detector *d,
                                            char c) {
    ...
    for (size_t i = 0; i < sizeof(markers) / sizeof(markers[0]); i++) {
        if (agent_tail_matches(d->tail, d->len, markers[i], strlen(markers[i])))
            return true;
    }
    return false;
}

static void agent_stream_note_thinking_qwen_tool_byte(agent_stream_renderer *sr,
                                                 char c) {
    if (!sr->in_think || sr->qwen_tool_in_think) return;
    if (agent_qwen_tool_marker_detector_feed(&sr->think_qwen_tool, c))
        sr->qwen_tool_in_think = true;
}

static void agent_stream_note_plain_qwen_tool_byte(agent_stream_renderer *sr,
                                              char c) {
    if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) return;
    if (sr->qwen_tool_active || sr->in_think || sr->qwen_tool_in_think) return;
    if (agent_qwen_tool_marker_detector_feed(&sr->plain_qwen_tool, c)) {
        agent_stream_malformed_qwen_tool(
            sr, "Qwen tool markup outside a valid tool_call block");
    }
}

/* Route ordinary assistant bytes either to normal markdown rendering or into
 * the Qwen tool detector. The opening marker may be split across tokens. */
static void agent_stream_normal_byte(agent_stream_renderer *sr, char c) {
    static const char start[] = "<tool_call>";
    if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) return;
    agent_stream_note_thinking_qwen_tool_byte(sr, c);

    /* Qwen may emit blank lines after </think> before prose or a tool call. They are
    ...
    if (sr->qwen_tool_start_len || c == start[0]) {
        if (sr->qwen_tool_start_len < sizeof(sr->qwen_tool_start_tail))
            sr->qwen_tool_start_tail[sr->qwen_tool_start_len++] = c;
        bool complete = false;
        if (agent_stream_qwen_tool_start_match(sr->qwen_tool_start_tail, sr->qwen_tool_start_len,
                                               &complete))
        {
            if (complete) {
                agent_stream_start_qwen_tool(sr, sr->in_think);
            }
            return;
        }
        if (sr->qwen_tool_start_len > 1 &&
            sr->qwen_tool_start_tail[sr->qwen_tool_start_len - 1] == start[0])
        {
            sr->post_think_gap = false;
            size_t flush = sr->qwen_tool_start_len - 1;
            for (size_t i = 0; i < flush; i++) {
                renderer_write_char(sr->renderer, sr->qwen_tool_start_tail[i]);
                agent_stream_note_plain_qwen_tool_byte(sr, sr->qwen_tool_start_tail[i]);
                if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) break;
            }
            if (sr->parser->state == AGENT_QWEN_TOOL_ERROR) {
                sr->qwen_tool_start_len = 0;
                return;
            }
            sr->qwen_tool_start_tail[0] = start[0];
            sr->qwen_tool_start_len = 1;
            return;
        }
    ...
    agent_stream_note_plain_qwen_tool_byte(sr, c);
}

/* This is the single streaming display state machine for assistant output.  It
 * hides raw Qwen tool as soon as the tool_calls marker is complete, lets the Qwen tool
...
static void agent_stream_text(agent_stream_renderer *sr, const char *text, size_t len, bool finish) {
     ...
     * between generated chunks.  If a Qwen tool parameter is still streaming, make
    ...
    if (len && !sr->qwen_tool_active) renderer_restore_text_attrs(sr->renderer);

    size_t i = 0;
    while (i < total) {
        char *cur = buf + i;
        size_t rem = total - i;
        if (!sr->qwen_tool_active && bytes_has_prefix(cur, rem, think_open)) {
            agent_stream_flush_start_tail(sr);
        ...
        if (!sr->qwen_tool_active && bytes_has_prefix(cur, rem, think_close)) {
            agent_stream_flush_start_tail(sr);
            renderer_hint_end(sr->renderer);
            sr->in_think = false;
        ...
        if (!finish && !sr->qwen_tool_active && cur[0] == '<' &&
            (bytes_is_partial_prefix(cur, rem, think_open) ||
             bytes_is_partial_prefix(cur, rem, think_close)))
        {
        ...
        if (sr->qwen_tool_active) {
            agent_stream_feed_qwen_tool_byte(sr, cur[0]);
        } else if (sr->in_think) {
            /* Tool calls are executable only after thinking has closed.  Still
             * route thinking bytes through the Qwen tool start detector so an
    ...
    if (finish) {
        ...
        if (sr->qwen_tool_in_think && !sr->qwen_tool_in_think_reported) {
            agent_stream_finish_ignored_qwen_tool(
                sr, "tool calling is not allowed inside <think></think>");
        }
    }
...
static char *agent_default_cache_dir(void) {
    const char *cache = getenv("Q36_AGENT_CACHE_DIR");
    if (cache && cache[0]) return xstrdup(cache);
    const char *home = getenv("HOME");
    if (!home || !home[0]) home = ".";
    agent_buf b = {0};
    agent_buf_puts(&b, home);
    if (b.len == 0 || b.ptr[b.len - 1] != '/') agent_buf_puts(&b, "/");
    agent_buf_puts(&b, ".q36/kvcache");
...
static char *agent_kv_path_for_sha(const char *dir, const char sha[41]) {
    ...
    memcpy(name, sha, 40);
    memcpy(name + 40, ".kv", 4);
    return q36_kvstore_path_join(dir, name);
}
...
static void agent_session_identity_sha(const char *title, uint64_t created_at,
    ...
    q36_kvstore_sha1_bytes_hex(b.ptr ? b.ptr : "", b.len, sha_out);
    free(b.ptr);
}
 ...
 *
 * The Q36 payload stores the exact token sequence and graph state.  The rendered
...
static bool agent_kv_write_title_trailer(FILE *fp, const char *title,
    ...
    uint8_t tb[4];
    q36_kvstore_le_put32(tb, (uint32_t)title_len);
 ...
 * start expected by q36_session_load_payload(). */
static bool agent_kv_read_title_trailer(FILE *fp, const q36_kvstore_entry *hdr,
                                        char **title_out,
                                        char *err, size_t err_len) {
...
static void agent_kv_identity_sha(const q36_kvstore_entry *hdr,
                                  const char *text, uint32_t text_bytes,
                                  const char *title,
                                  char sha_out[41]) {
    if (hdr->ext_flags & Q36_KVSTORE_EXT_SESSION_TITLE) {
        agent_session_identity_sha(title ? title : "", hdr->created_at, sha_out);
    } else {
        q36_kvstore_sha1_bytes_hex(text, text_bytes, sha_out);
    }
}
...
static bool agent_kv_load_path(agent_worker *w, const char *path,
    ...
    char load_err[160] = {0};
    if (ok && hdr.payload_bytes == 0) {
        q36_tokens rebuilt = {0};
        q36_tokenize_rendered_chat(w->engine, text, &rebuilt);
        ...
        q36_tokens_free(&rebuilt);
    } else if (ok &&
               q36_session_load_payload(w->session, fp, hdr.payload_bytes,
    ...
    if (ok) {
        const q36_tokens *live = q36_session_tokens(w->session);
        if (!live || live->len != (int)expected_tokens) {
            snprintf(err, err_len, "KV payload token count mismatch");
            q36_session_invalidate(w->session);
            ok = false;
        } else if (loaded_tokens) {
            q36_tokens_free(loaded_tokens);
            q36_tokens_copy(loaded_tokens, live);
        }
        if (meta_out) {
...
static bool agent_kv_save_path(agent_worker *w, const char *path,
    ...
    size_t text_len = 0;
    char *text = q36_kvstore_render_tokens_text(w->engine, tokens, &text_len);
    if (!text) {
    ...
    if (session_identity)
        ...
        q36_kvstore_sha1_bytes_hex(text, text_len, sha);
    if (sha_out) memcpy(sha_out, sha, sizeof(sha));

    q36_session_payload_file staged = {0};
    char save_err[160] = {0};
    if (q36_session_stage_payload(w->session, &staged,
    ...
    if (fd < 0) {
        snprintf(err, err_len, "%s", strerror(errno));
        q36_session_payload_file_free(&staged);
        free(tmp);
        free(text);
        return false;
    }
    ...
    uint8_t h[Q36_KVSTORE_FIXED_HEADER];
    q36_kvstore_fill_header(h, (uint8_t)model_id, (uint8_t)quant_bits,
                            q36_kvstore_reason_code(reason),
                            session_identity ? Q36_KVSTORE_EXT_SESSION_TITLE : 0,
                            (uint32_t)tokens->len, 0,
                            (uint32_t)q36_session_ctx(w->session),
                            created_at, now, payload_bytes);
    uint8_t tb[4];
    q36_kvstore_le_put32(tb, (uint32_t)text_len);

    errno = 0;
    bool ok = fwrite(h, 1, sizeof(h), fp) == sizeof(h) &&
              fwrite(tb, 1, sizeof(tb), fp) == sizeof(tb) &&
              fwrite(text, 1, text_len, fp) == text_len &&
              q36_session_write_staged_payload(&staged, fp,
                                               save_err, sizeof(save_err)) == 0 &&
              (!session_identity ||
...
static bool agent_worker_build_system_tokens(agent_worker *w, q36_tokens *out) {
    ...
    agent_append_system_prompt(w->engine, out, w->cfg->gen.system, &thinking,
                               w->cfg->edit_upto);
    q36_prompt_prefix_append(w->engine, out, &w->cfg->gen.prefix);
    return thinking;
}
...
static int agent_web_confirm(void *privdata, const char *message,
    ...
    if (!w || w->cfg->non_interactive) {
        snprintf(err, err_len,
                 "visible browser startup requires interactive approval");
        return 0;
    }
    ...
    snprintf(w->web_approval_message, sizeof(w->web_approval_message),
             "%s", message ? message : "Start visible browser? (y/n) ");
    agent_wake_locked(w);
...
/* Synchronize the live Q36 session to a transcript.  This is the agent's main
 * cache-saving operation: if the requested transcript extends the live session,
 * only the suffix is prefetched; otherwise the Q36 session rebuilds from the
 * longest common prefix it can retain. */
static int agent_worker_sync_tokens(agent_worker *w, const q36_tokens *tokens,
                                    bool publish_progress,
                                    char *err, size_t err_len) {
    int old_pos = q36_session_pos(w->session);
    int common = q36_session_common_prefix(w->session, tokens);
    ...
    q36_session_set_progress(w->session, publish_progress ? worker_progress_cb : NULL,
                             publish_progress ? w : NULL);
    q36_session_set_display_progress(w->session,
                                     publish_progress ? worker_progress_cb : NULL,
                                     publish_progress ? w : NULL);
    q36_session_set_cancel(w->session, worker_cancel_session_cb, w);
    int rc = w->image_count ?
        q36_session_sync_vision(w->session, tokens, w->images, w->image_count, err, err_len) :
        q36_session_sync(w->session, tokens, err, err_len);
 ...
 * by model variants; agent_kv_load_path() checks the model id, so switching
...
static bool agent_worker_reset_to_sysprompt(agent_worker *w, char *err, size_t err_len) {
    ...
    if (!loaded) {
        ...
        q36_tokens_copy(&w->transcript, &sys);
        ...
        if (w->sysprompt_path) {
                                    ...
                                    save_err, sizeof(save_err)))
            {
                if (w->cfg->non_interactive) {
                    fprintf(stderr, "q36-agent: failed to save system prompt KV: %s\n",
                            save_err);
                } else {
                    agent_buf b = {0};
                    agent_buf_puts(&b, "\nq36-agent: failed to save system prompt KV: ");
...
static bool agent_worker_save_session_now(agent_worker *w, char sha_out[41],
    ...
    size_t text_len = 0;
    char *text = q36_kvstore_render_tokens_text(w->engine, &w->transcript,
                                                &text_len);
    if (!text) {
...
static char *agent_session_title_from_file(const char *path, size_t max_bytes) {
    ...
    if (ok && (hdr.ext_flags & Q36_KVSTORE_EXT_SESSION_TITLE))
...
static const char *agent_history_start_for_turns(const char *text, size_t len,
    ...
    if (users.len > 0) {
         ...
         * assistant Qwen tool stanza that produced them.  Include that preceding
...
static bool agent_history_latest_compaction_summary(const char *text,
    ...
    static const char start_mark[] =
        "[q36-agent compacted earlier conversation. Durable task-state summary follows.]";
...
static void agent_history_render_assistant(agent_worker *w,
    ...
    agent_qwen_tool_parser qwen_tool = {.state = AGENT_QWEN_TOOL_SEARCH};
    ...
    /* Dry-run replay: the same streaming projection hides Qwen tool and renders
...
static void agent_session_list_free(agent_session_list_item *v, int n) {
    for (int i = 0; i < n; i++) {
        q36_kvstore_entry_free(&v[i].entry);
        free(v[i].title);
    }
    free(v);
}
...
static void agent_session_list_push(agent_session_list_item **v, int *len,
                                    int *cap, q36_kvstore_entry entry,
                                    char *title) {
    if (*len == *cap) {
        *cap = *cap ? *cap * 2 : 16;
...
/* Print resumable sessions from ~/.q36/kvcache.  sysprompt.kv is intentionally
...
static void agent_worker_list_sessions(agent_worker *w) {
    ...
    const uint8_t model_id = (uint8_t)q36_engine_model_id(w->engine);
    struct dirent *de;
    while ((de = readdir(d)) != NULL) {
        char sha[41];
        if (!q36_kvstore_sha_hex_name(de->d_name, sha)) continue;
        char *path = q36_kvstore_path_join(w->cache_dir, de->d_name);
        q36_kvstore_entry e = {0};
        if (q36_kvstore_read_entry_file(path, sha, &e)) {
            if (e.model_id == model_id) {
                char *title = agent_session_title_from_file(path, title_budget);
                agent_session_list_push(&sessions, &sessions_len, &sessions_cap,
                                        e, title);
            } else {
                q36_kvstore_entry_free(&e);
            }
        }
        free(path);
    }
    ...
    for (int i = 0; i < sessions_len; i++) {
        q36_kvstore_entry *e = &sessions[i].entry;
        char age[32];
...
static bool agent_worker_find_session(agent_worker *w, const char *prefix,
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        char *path = q36_kvstore_path_join(w->cache_dir, de->d_name);
        q36_kvstore_entry e = {0};
        bool same_model = q36_kvstore_read_entry_file(path, sha, &e) &&
                          e.model_id == model_id;
        q36_kvstore_entry_free(&e);
        if (!same_model) {
            free(path);
...
static bool agent_worker_strip_session(agent_worker *w, const char *prefix,
    ...
    q36_tokens stripped_tokens = {0};
    q36_tokenize_rendered_chat(w->engine, text, &stripped_tokens);
    uint32_t stripped_token_count = (uint32_t)stripped_tokens.len;
    q36_tokens_free(&stripped_tokens);
    ...
    uint8_t h[Q36_KVSTORE_FIXED_HEADER];
    uint64_t now = (uint64_t)time(NULL);
    q36_kvstore_fill_header(h, hdr.model_id, hdr.quant_bits, hdr.reason, hdr.ext_flags,
                            stripped_token_count, hdr.hits, hdr.ctx_size,
                            hdr.created_at, now, 0);
    uint8_t tb[4];
    q36_kvstore_le_put32(tb, text_bytes);

    errno = 0;
    ok = fwrite(h, 1, sizeof(h), fp) == sizeof(h) &&
         fwrite(tb, 1, sizeof(tb), fp) == sizeof(tb) &&
         fwrite(text, 1, text_bytes, fp) == text_bytes &&
         (!(hdr.ext_flags & Q36_KVSTORE_EXT_SESSION_TITLE) ||
...
static bool agent_worker_switch_session(agent_worker *w, const char *prefix,
    ...
    bool stripped = false;
    q36_kvstore_entry entry = {0};
    if (q36_kvstore_read_entry_file(path, sha, &entry)) {
        stripped = entry.payload_bytes == 0;
        q36_kvstore_entry_free(&entry);
    }
    if (stripped) {
...
static FILE *agent_open_regular_file(const char *path) {
    ...
    int fd = open(expanded, O_RDONLY | O_NONBLOCK | O_CLOEXEC);
    if (fd < 0) return NULL;
    struct stat st;
    int rc = fstat(fd, &st);
...
static int agent_replace_file(const char *path, const char *data, size_t len,
    ...
    char expanded[PATH_MAX];
    if (!agent_expand_home_path(path, expanded, sizeof(expanded))) {
        snprintf(err, errlen, "path is too long");
        return -1;
    }
    path = expanded;
    ...
    if (exists) {
        ...
        if (!S_ISREG(before.st_mode) || before.st_nlink != 1) {
            snprintf(err, errlen, "refusing to replace non-regular or hard-linked file: %.160s", path);
            goto done;
        }
        if (expected) {
    ...
    if (snprintf(temp, sizeof(temp), "%s.q36-XXXXXX", target) >= (int)sizeof(temp)) {
        temp[0] = 0;
        errno = ENAMETOOLONG;
        goto failed;
    }
    ...
    goto done;
changed:
    snprintf(err, errlen, "file changed while editing; read it again: %.160s", path);
    goto done;
failed:
    snprintf(err, errlen, "replace %.160s: %.64s", path, strerror(errno));
done:
    if (fd >= 0) close(fd);
    if (src >= 0) close(src);
...
static bool agent_edit_upto_forcer_should_replace(agent_edit_upto_forcer *forcer,
                                                  agent_qwen_tool_parser *p,
                                                  const char *next_text,
                                                  size_t next_len) {
    if (!forcer || !p) return false;
    bool in_edit_old = p->state == AGENT_QWEN_TOOL_PARAM_VALUE &&
...
static void test_agent_qwen_tool_parser(void) {
    const char *raw =
        "<tool_call>\n"
        "<function=edit>\n"
        "<parameter=path>\n/tmp/a.c\n</parameter>\n"
        ...
        "</function>\n"
        "</tool_call>";
    agent_qwen_tool_parser p = {.state = AGENT_QWEN_TOOL_SEARCH};
    for (size_t i = 0; raw[i]; i++) agent_qwen_tool_feed(&p, raw + i, 1);
    AGENT_TEST_ASSERT(p.state == AGENT_QWEN_TOOL_DONE);
    AGENT_TEST_ASSERT(p.calls.len == 1);
    AGENT_TEST_ASSERT(!strcmp(p.calls.v[0].name, "edit"));
    AGENT_TEST_ASSERT(!strcmp(agent_tool_arg_value(&p.calls.v[0], "path"), "/tmp/a.c"));
    AGENT_TEST_ASSERT(!strcmp(agent_tool_arg_value(&p.calls.v[0], "old"),
                              "line one\nline two"));
    AGENT_TEST_ASSERT(!strcmp(agent_tool_arg_value(&p.calls.v[0], "new"),
                              "replacement"));
    agent_qwen_tool_parser_free(&p);
}
...
static void test_agent_chat_template_controls(void) {
    ...
    char *system = agent_build_system_prompt(
        "<|think_off|> custom system instructions", &thinking, false, false);
    AGENT_TEST_ASSERT(!thinking);
...
static void test_agent_edit_upto_prompt_is_opt_in(void) {
    char *exact = agent_build_tools_prompt(false, false);
    char *upto = agent_build_tools_prompt(true, true);
    ...
    AGENT_TEST_ASSERT(strstr(exact, "view_image") == NULL);
    AGENT_TEST_ASSERT(strstr(upto, "view_image") != NULL);
    free(exact);
    free(upto);
}
...
static void test_agent_streaming_defaults(void) {
    ...
    char *streamctxv[] = {"q36-agent", "--ctx", "65536", "--ssd-streaming"};
    char *ctxstreamv[] = {"q36-agent", "--ssd-streaming", "--ctx", "49152"};
    ...
    agent_config ctxstream = parse_options(4, ctxstreamv);
...
#else
    AGENT_TEST_ASSERT(def.engine.cache_type_k == Q36_KV_CACHE_Q8_0);
    AGENT_TEST_ASSERT(def.engine.cache_type_v == Q36_KV_CACHE_Q4_0);
#endif
    ...
    AGENT_TEST_ASSERT(streamctx.gen.ctx_size == 65536);
    AGENT_TEST_ASSERT(ctxstream.engine.ssd_streaming);
    AGENT_TEST_ASSERT(ctxstream.gen.ctx_size == 49152);
    AGENT_TEST_ASSERT(streamcache.engine.cache_type_k == Q36_KV_CACHE_Q8_0);
    AGENT_TEST_ASSERT(streamcache.engine.cache_type_v == Q36_KV_CACHE_Q4_0);
    AGENT_TEST_ASSERT(cache.engine.cache_type_k == Q36_KV_CACHE_Q4_0);
    AGENT_TEST_ASSERT(cache.engine.cache_type_v == Q36_KV_CACHE_F16);
...
static void test_agent_hints_state(void) {
    ...
    char *tools = agent_build_tools_prompt(false, false);
    AGENT_TEST_ASSERT(strstr(tools, "programming hints") == NULL);
    free(tools);
}
...
static char *agent_tool_search(agent_worker *w, const agent_tool_call *call) {
    ...
    struct stat root;
    if (stat(expanded, &root) != 0 || (!S_ISDIR(root.st_mode) && !S_ISREG(root.st_mode)))
        return xstrdup("Tool error: search path is missing, unreadable, or not a regular file/directory\n");
 ...
 * ============================================================================
 *
 * The browser subsystem lives in q36_web.c: it owns a visible browser and CDP.  The
...
static char *agent_tool_google_search(agent_worker *w, const agent_tool_call *call) {
    ...
    char *md = q36_web_google_search(w->web, query, err, sizeof(err));
    if (!md) {
        agent_buf b = {0};
...
static char *agent_tool_visit_page(agent_worker *w, const agent_tool_call *call) {
    ...
    char *md = q36_web_visit_page(w->web, url, err, sizeof(err));
    if (!md) {
        agent_buf b = {0};
    ...
    char path[PATH_MAX];
    if (!agent_write_temp_text("q36_agent_web", md, path, sizeof(path),
                               err, sizeof(err)))
    {
        free(md);
        agent_buf b = {0};
...
static void agent_bash_drain_terminal(agent_bash_job *job) {
    ...
    if (!agent_request_password(job->worker, &request)) {
        ...
        if (job->running && job->tmp_fd >= 0) {
            agent_bash_note_output(job, msg, strlen(msg));
            if (write(job->tmp_fd, msg, strlen(msg)) < 0) job->output_error = errno;
        }
        pthread_mutex_unlock(&job->mu);
        agent_bash_signal(job, SIGKILL);
    }
...
static agent_bash_job *agent_bash_start(agent_worker *w, const char *cmd,
    ...
    job->running = true;
    job->worker = w;
    int rc = pthread_create(&job->thread, NULL, agent_bash_monitor, job);
    if (rc != 0) {
        snprintf(err, err_len, "failed to start shell monitor: %s", strerror(rc));
        agent_bash_job_free(job);
        unlink(tmp_path);
        return NULL;
    }
...
static void agent_tool_observation_free(agent_tool_observation *obs) {
    ...
    for (size_t i = 0; i < obs->image_count; i++)
        q36_vision_embedding_free(&obs->images[i]);
    free(obs->parts);
    free(obs->images);
    memset(obs, 0, sizeof(*obs));
}
...
static void agent_tool_view_image(agent_worker *w,
    ...
    if (!q36_engine_has_vision(w->engine)) {
        agent_tool_observation_puts(
            obs, "Tool error: view_image requires q36-agent --vision FILE\n");
        return;
    }
    q36_vision_embedding embedding = {0};
    char err[256] = {0};
    char expanded[PATH_MAX];
    int encoded = agent_expand_home_path(path, expanded, sizeof(expanded)) &&
        q36_engine_vision_encode_file(w->engine, expanded, &embedding, err, sizeof(err));
    if (!encoded) {
...
static void agent_vision_spans_free(q36_vision_span *spans, size_t count) {
    if (!spans) return;
    for (size_t i = 0; i < count; i++)
        q36_vision_embedding_free(&spans[i].embedding);
    free(spans);
}
...
static bool agent_tool_observation_build(agent_worker *w,
    ...
    q36_vision_embedding *images = NULL;
    q36_vision_span *spans = NULL;
    if (obs->image_count) {
        ...
        const int n_embd = q36_engine_embd_dim(w->engine);
        for (size_t i = 0; i < obs->image_count; i++) {
            const q36_vision_embedding *src = &obs->images[i];
            if (!src->data || src->token_count == 0 || n_embd <= 0 ||
                ...
                for (size_t j = 0; j < i; j++)
                    q36_vision_embedding_free(&images[j]);
                free(images);
                free(spans);
                free(parts);
    ...
    q36_tokens_copy(tokens, &w->transcript);
    bool ok = q36_chat_append_multimodal_message(
        w->engine, tokens, "tool",
        parts, images, obs->image_count, spans,
        err, err_len) != 0;
    free(parts);
    if (!ok) {
        for (size_t i = 0; i < obs->image_count; i++) {
            q36_vision_embedding_free(&images[i]);
            q36_vision_embedding_free(&spans[i].embedding);
        }
    }
    free(images);
...
static int agent_tool_observation_fits(agent_worker *w,
    ...
    if (reserve_tokens < compact) reserve_tokens = compact;
    return ctx > 0 && tokens < ctx - reserve_tokens;
}
...
static bool agent_worker_should_compact(agent_worker *w) {
    int ctx = w->cfg->gen.ctx_size, used = w->transcript.len;
    return agent_context_should_compact(ctx, used) ||
           (used > 0 && ctx - used <= agent_compact_reserve_tokens(w) + 128);
}

static int agent_special_token_id(q36_engine *engine, const char *rendered) {
    q36_tokens t = {0};
    q36_tokenize_rendered_chat(engine, rendered, &t);
    int id = t.len == 1 ? t.v[0] : -1;
    q36_tokens_free(&t);
    return id;
}

static int agent_compact_tail_boundary(const q36_tokens *tokens, int bottom,
                                       int sys_len, int budget,
                                       const q36_tokens *user) {
    int target = bottom - budget;
    if (target < sys_len) target = sys_len;
    int earliest = bottom - 2 * budget;
    if (earliest < sys_len) earliest = sys_len;
    int found = -1;
    for (int i = earliest; i + user->len <= bottom && user->len > 0; i++) {
        if (!memcmp(tokens->v + i, user->v, (size_t)user->len * sizeof(int)))
            found = i;
    }
    return found >= 0 ? found : target;
}

/* Retain the most recent complete user turn when it fits within twice the tail budget. */
static int agent_compact_tail_start(agent_worker *w, int bottom, int sys_len) {
    int budget = w->cfg->gen.ctx_size / AGENT_COMPACT_TAIL_DIVISOR;
    if (budget > AGENT_COMPACT_TAIL_CAP_TOKENS) budget = AGENT_COMPACT_TAIL_CAP_TOKENS;
    if (budget < 1) budget = 1;
    q36_tokens user = {0};
    q36_tokenize_rendered_chat(w->engine, "<|im_start|>user\n", &user);
    int start = agent_compact_tail_boundary(&w->transcript, bottom, sys_len, budget, &user);
    q36_tokens_free(&user);
    return start;
}

static void agent_tokens_append_range(q36_tokens *dst, const q36_tokens *src,
                                      int start, int end) {
    if (start < 0) start = 0;
    if (end > src->len) end = src->len;
    for (int i = start; i < end; i++) q36_tokens_push(dst, src->v[i]);
}
...
static char *agent_compact_make_prompt(const char *reason) {
    agent_buf b = {0};
    agent_buf_puts(&b,
        "Internal q36-agent context compaction request. This is not a user request.\n"
        ...
        "After the summary, stop. Do not continue the user task, do not call tools, and do not output thinking tags or Qwen tool markup.\n"
...
static int agent_compact_reserve_tokens(agent_worker *w) {
    if (w->compact_reserve) return w->compact_reserve;
    char *text = agent_compact_make_prompt("context pressure before continuing the current task");
    q36_tokens tokens = {0};
    q36_chat_append_message(w->engine, &tokens, "user", text);
    q36_chat_append_assistant_prefix(w->engine, &tokens, Q36_THINK_NONE);
    int reserve = tokens.len + 64 +
                  agent_compact_summary_budget(w->cfg->gen.ctx_size);
    free(text);
    q36_tokens_free(&tokens);
    w->compact_reserve = reserve;
    return reserve;
}

static int agent_compact_image_boundary(const q36_vision_span *images, size_t count,
                                        int pos) {
    for (size_t i = 0; i < count; i++) {
        const q36_vision_span *span = &images[i];
        int wrapper = 1;
...
/* Perform the full compaction exchange and rebuild the live Q36 session from
...
static bool agent_worker_compact_transcript(agent_worker *w, const char *reason,
    ...
    if (summary_bottom <= sys.len) {
        ...
        q36_tokens_free(&summary_suffix);
        q36_tokens_free(&sys);
        agent_publish(w, "\x1b[0m\n", 5);
        return false;
    }
    ...
    /* A bounded prefix may end inside a user message, thinking, or code.
     * Close it only in the private summary prompt; the real tail is untouched. */
    int im_start = agent_special_token_id(w->engine, "<|im_start|>");
    int im_end = agent_special_token_id(w->engine, "<|im_end|>");
    int think_start = agent_special_token_id(w->engine, "<think>");
    int think_end = agent_special_token_id(w->engine, "</think>");
    bool message_open = false, think_open = false;
    for (int i = 0; i < prompt.len; i++) {
        int token = prompt.v[i];
        if (token == im_start) { message_open = true; think_open = false; }
        if (token == im_end) { message_open = false; think_open = false; }
        if (token == think_start) think_open = true;
        if (token == think_end) think_open = false;
    }
    if (message_open) {
        if (think_open) q36_tokenize_rendered_chat(w->engine, "</think>\n", &prompt);
        q36_tokenize_rendered_chat(w->engine, "<|im_end|>\n", &prompt);
    }
    ...
    int sync_rc;
    if (summary_images) {
        sync_rc = q36_session_sync_vision(w->session, &prompt,
                                         w->images, summary_images, err, err_len);
    } else {
        sync_rc = q36_session_sync(w->session, &prompt, err, err_len);
    }
    q36_session_set_cancel(w->session, NULL, NULL);
    q36_session_set_progress(w->session, NULL, NULL);
    q36_session_set_display_progress(w->session, NULL, NULL);
    if (sync_rc == Q36_SESSION_SYNC_INTERRUPTED) {
        q36_session_invalidate(w->session);
    ...
    for (int i = 0; i < summary_max; i++) {
        ...
        int token = q36_session_argmax(w->session);
        ...
        if (q36_session_eval(w->session, token, eval_err, sizeof(eval_err)) != 0) {
        ...
        free(text);
        char *tool = summary.ptr ? strstr(summary.ptr, "<tool_call>") : NULL;
        if (tool) {
            summary.len = (size_t)(tool - summary.ptr);
            summary.ptr[summary.len] = '\0';
            agent_trace(w, "compaction summary stopped before tool call");
            break;
        }
    ...
    tail_start = agent_compact_image_boundary(w->images, w->image_count, tail_start);
    q36_tokens compacted = {0};
    q36_tokens_copy(&compacted, &sys);
    ...
    q36_chat_append_message(w->engine, &compacted, "user", summary_msg.ptr);
    ...
    if (open_assistant && tail_start > *open_assistant) {
        ...
        q36_chat_append_assistant_prefix(w->engine, &compacted,
            in_think ? Q36_THINK_HIGH : Q36_THINK_NONE);
    } else if (tail_start < bottom &&
        w->transcript.v[tail_start] != agent_special_token_id(w->engine, "<|im_start|>")) {
        q36_chat_append_message(w->engine, &compacted, "user",
    ...
    q36_vision_span *old_images = w->images;
    size_t old_image_count = w->image_count;
    size_t old_image_cap = w->image_cap;
    q36_vision_span *new_images = old_image_count ?
    ...
    q36_tokens old_transcript = {0};
    q36_tokens_copy(&old_transcript, &w->transcript);
    q36_tokens_free(&w->transcript);
    ...
    for (size_t i = 0; i < old_image_count; i++) {
        if (!kept_images[i]) q36_vision_embedding_free(&old_images[i].embedding);
    }
    free(old_images);
    free(kept_images);
    pthread_mutex_lock(&w->mu);
    ...
    pthread_mutex_unlock(&w->mu);
    agent_worker_note_system_prompt_seen(w);
    q36_tokens_free(&old_transcript);
    q36_tokens_free(&sys);
    ...
    if (bash_update) {
        q36_chat_append_message(w->engine, &w->transcript, "tool", bash_update);
        w->session_dirty = true;
...
static bool agent_worker_user_fits(agent_worker *w, const q36_tokens *message,
                                   char *err, size_t err_len) {
    q36_tokens sys = {0};
    ...
    q36_tokens_free(&sys);
    if (!ok) snprintf(err, err_len,
        "user message is too large for --ctx %d; use a larger context or a smaller message", ctx);
    return ok;
}

static bool agent_worker_append_user(agent_worker *w, const char *text,
                                     char *err, size_t err_len) {
    q36_tokens message = {0};
    q36_chat_append_message(w->engine, &message, "user", text);
    bool ok = agent_worker_user_fits(w, &message, err, err_len);
    int ctx = w->cfg->gen.ctx_size;
...
static int worker_accept_generated_token(agent_worker *w,
                                         ...
                                         char *err,
                                         size_t err_len) {
    if (q36_session_eval(w->session, token, err, err_len) != 0)
        return 1;

    q36_tokens_push(&w->transcript, token);
...
static int worker_force_generated_text(agent_worker *w,
                                       ...
                                       size_t err_len) {
    q36_tokens tokens = {0};
    q36_tokenize_text(w->engine, text, &tokens);
...
/* Qwen tool structure is a machine-readable grammar, so once the model has clearly
 ...
 * parameter closing tag once it is clearly Qwen tool syntax, not ordinary text such
...
static bool agent_stream_wants_greedy_sampling(const agent_stream_renderer *sr) {
    ...
    if (sr->parser->state == AGENT_QWEN_TOOL_ERROR ||
        sr->parser->state == AGENT_QWEN_TOOL_DONE)
        return false;
     ...
     * Qwen tool-shaped ("<tool_call..."). */
    if (sr->qwen_tool_start_len > 1) return true;
    if (!sr->qwen_tool_active) return false;

    if (sr->parser->state == AGENT_QWEN_TOOL_STRUCTURAL)
        return true;
    if (sr->parser->state != AGENT_QWEN_TOOL_PARAM_VALUE)
        return false;
...
static bool agent_stream_compaction_needs_lookahead(const agent_stream_renderer *sr) {
    return !sr->qwen_tool_active && sr->parser->state == AGENT_QWEN_TOOL_SEARCH &&
...
static int worker_run_turn(agent_worker *w, const char *user_text) {
    ...
    for (int tool_round = 0; ; tool_round++) {
        ...
        int old_pos = q36_session_pos(w->session);
        int common = q36_session_common_prefix(w->session, &w->transcript);
        ...
        agent_trace(w, "prefill tool_round=%d transcript=%d prompt=%d cached=%d suffix=%d think=%s",
                    ...
                    cached, suffix, q36_think_mode_name(think_mode));
        ...
        agent_trace(w, "prefill sync done tool_round=%d pos=%d", tool_round,
                    q36_session_pos(w->session));
        int room = q36_session_ctx(w->session) - q36_session_pos(w->session);
        ...
        while (generated < max_tokens && !worker_should_interrupt(w)) {
            ...
            if (token == q36_token_eos(w->engine)) {
                model_stopped = true;
                break;
            }
            ...
            if (qwen_tool.state == AGENT_QWEN_TOOL_DONE) {
                got_tool = true;
                break;
            }
            ...
            if (qwen_tool.state == AGENT_QWEN_TOOL_ERROR) {
                malformed_tool = true;
                break;
            }
            ...
            if (context_limited && generated == max_tokens &&
                compaction_lookahead < 32 && agent_stream_compaction_needs_lookahead(&stream)) {
                max_tokens++;
        ...
        if (stream.qwen_tool_in_think) {
            ...
            snprintf(qwen_tool.error, sizeof(qwen_tool.error),
                     "tool calling is not allowed inside <think></think>");
        } else if (!malformed_tool && qwen_tool.state == AGENT_QWEN_TOOL_ERROR) {
            malformed_tool = true;
        } else if (!got_tool && !malformed_tool && !early_tool_error &&
                   !interrupted &&
                   (qwen_tool.state == AGENT_QWEN_TOOL_STRUCTURAL ||
        ...
        if (early_tool_error) {
            ...
            agent_tool_observation_puts(
                &observation, "Tool error: invalid Qwen tool call: ");
            agent_tool_observation_puts(
        ...
        for (size_t i = 0; i < observation.part_count; i++)
            agent_buf_puts(&observed_text, observation.parts[i].text ? observation.parts[i].text : "");
        char *warning = agent_tool_response_add_warning(agent_buf_take(&observed_text),
                                                       &consecutive_failures);
        if (consecutive_failures > 0) {
            const char *start = strstr(warning, "\n\n⚠️ SYSTEM WARNING:");
        ...
        if (queued_user && queued_user[0]) {
            ...
            if (!agent_worker_append_user(w, content, compact_err, sizeof(compact_err))) {
                free(content);
                free(queued_user);
                if (agent_err_is_interrupted(compact_err)) {
                    worker_clear_interrupt(w);
...
static void worker_apply_pending_power(agent_worker *w) {
    ...
    if (q36_session_set_power(w->session, power) != 0) {
        agent_publishf(w, "\npower change failed\n");
        return;
    }
    pthread_mutex_lock(&w->mu);
...
static void *worker_main(void *arg) {
    ...
    agent_trace(w, "agent worker start ctx=%d backend=%s model=%s trace=%s",
                w->cfg->gen.ctx_size,
                q36_backend_name(w->cfg->engine.backend),
...
static void editor_clear_prompt_region(agent_editor *ed) {
    ...
    /* In scroll-region mode q36-agent owns the absolute prompt/status rows.
 ...
 * and model thread.  After this returns, all Q36 session mutation happens on
 * the worker thread. */
static int agent_worker_init(agent_worker *w, q36_engine *engine, agent_config *cfg) {
    memset(w, 0, sizeof(*w));
    w->engine = engine;
    w->cfg = cfg;
    w->wake_fd[0] = -1;
    ...
    if (!agent_mkdir_p(w->cache_dir)) {
        fprintf(stderr, "q36-agent: failed to create %s: %s\n",
                w->cache_dir, strerror(errno));
        return -1;
    }
    q36_web_config web_cfg = {
    ...
    w->web = q36_web_create(&web_cfg);
    w->sysprompt_path = q36_kvstore_path_join(w->cache_dir, "sysprompt.kv");
    if (cfg->gen.trace_path && cfg->gen.trace_path[0]) {
        w->trace = fopen(cfg->gen.trace_path, "ab");
        if (!w->trace) {
            fprintf(stderr, "q36-agent: failed to open trace %s: %s\n",
...
static int run_agent_non_interactive(q36_engine *engine, agent_config *cfg) {
    agent_worker worker;
    ...
    if (!one_shot) {
        if (set_nonblock(STDIN_FILENO, true, &old_stdin_flags) != 0) {
            perror("q36-agent: nonblocking stdin");
    ...
    while (true) {
        ...
        {
            agent_noninteractive_marker("+QUARKSTAR_WAITING");
            ...
            (stdin_eof || now_sec() >= quiet_deadline))
        {
            ...
            if (worker_is_idle(&worker) && queue.len == 0) {
                if (!worker_submit(&worker, prompt)) {
                    ...
                    agent_noninteractive_marker("+QUARKSTAR_QUEUED");
                }
            } else {
                agent_prompt_queue_push(&queue, prompt);
                agent_noninteractive_marker("+QUARKSTAR_QUEUED");
...
static int run_agent(q36_engine *engine, agent_config *cfg) {
    agent_worker worker;
    if (agent_worker_init(&worker, engine, cfg) != 0) return 1;
    ...
    const char *home = getenv("HOME");
    if (!home || !home[0]) home = ".";
    snprintf(hist, sizeof(hist), "%s/.q36_agent_history", home);
    ...
    if (editor_start(&editor, prompt, statusline, NULL) != 0) {
        fprintf(stderr, "q36-agent: failed to start line editor\n");
        agent_worker_free(&worker);
        return 1;
    }
    ...
    while (running) {
                                             ...
                                             sizeof(web_approval_msg)))
        {
            ...
            worker_answer_web_approval(&worker, allow,
                approval_timed_out ? "browser start approval timed out" : NULL);
            worker_get_status(&worker, &st);
            build_prompt_text(&st, prompt, sizeof(prompt));
