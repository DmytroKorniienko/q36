#ifndef Q36_TOOL_TEXT_H
#define Q36_TOOL_TEXT_H
...
static inline bool q36_tool_text_escaped_close(const char *s, const char *end) {
    if (*s != '&') return false;
    s++;
...
static inline bool q36_tool_text_needs_escape(const char *s, const char *end) {
    return !strncmp(s, end, strlen(end)) || q36_tool_text_escaped_close(s, end);
}

static inline void q36_tool_text_unescape(char *s, const char *end) {
    char *out = s;
    while (*s) {
        if (q36_tool_text_escaped_close(s, end)) {
            if (!strncmp(s, "&amp;", 5)) {
                *out++ = '&';
                s += 5;
            } else {
...
static inline bool q36_tool_text_partial_escape(const char *s, size_t n,
                                                const char *end) {
    if (!n || *s != '&') return false;
    s++;
    n--;
