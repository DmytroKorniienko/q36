...
int q36_prompt_prefix_parse(q36_prompt_prefix *out,
    ...
    while (pos < len) {
        ...
        if (!prefix_marker_at(data, len, pos, &role, &marker_len)) {
            ...
            q36_prompt_prefix_free(out);
            return -1;
        }

        const q36_prompt_prefix_role expected =
            (out->count % 2 == 0) ? Q36_PROMPT_PREFIX_USER
                                  : Q36_PROMPT_PREFIX_ASSISTANT;
        if (role != expected) {
            prefix_error(error, error_cap, "line %zu: expected %s:", line,
                         expected == Q36_PROMPT_PREFIX_USER ? "USER"
                                                            : "ASSISTANT");
            q36_prompt_prefix_free(out);
            return -1;
        }
        ...
        for (size_t i = content_start; i < len; i++) {
            if (data[i] != '\n') continue;
            scan_line++;
            q36_prompt_prefix_role ignored_role;
            size_t ignored_len;
            if (i + 1 < len &&
        ...
        if (content_end == content_start) {
            prefix_error(error, error_cap, "line %zu has an empty %s turn",
                         line, role == Q36_PROMPT_PREFIX_USER ? "USER"
                                                              : "ASSISTANT");
            q36_prompt_prefix_free(out);
            return -1;
        }
        ...
        out->turns[out->count++] = (q36_prompt_prefix_turn) {
            .role = role,
            .content = content,
        };
    ...
    if (out->turns[out->count - 1].role != Q36_PROMPT_PREFIX_ASSISTANT) {
        prefix_error(error, error_cap,
...
int q36_prompt_prefix_load(q36_prompt_prefix *out,
    ...
    int rc = q36_prompt_prefix_parse(out, data, len, error, error_cap);
    free(data);
